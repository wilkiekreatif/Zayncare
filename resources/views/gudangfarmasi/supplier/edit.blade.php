          ...
          <div class="card card-default">
            ...
            <form action="{{route('supplier.update', $supplier->id)}}" method="POST">
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-sm-12">
                    <input value="{{$supplier->id}}" id="id" name="id" type="hidden" class="form-control">
                    <!-- text input -->
                    <div class="form-group">
                      <label class="col-form-label" for="suppliernama">Nama Supplier <a style="color:red">*</a></label>
                      <input type="text" class="form-control {{ $errors->has('suppliernama') ? 'is-invalid' : '' }}" name="suppliernama" id="suppliernama" placeholder="Silahkan isi ..." maxlength="30" value="{{$supplier->supplier_nama}}">
                    </div>
                    <div class="form-group">
                      <label class="col-form-label" for="supplieralamat"> Alamat Supplier <a style="color:red">*</a></label>
                      <textarea type="text" class="form-control {{ $errors->has('supplieralamat') ? 'is-invalid' : '' }}" name="supplieralamat" id="supplieralamat" placeholder="Silahkan isi ..." maxlength="160" rows="2">{{$supplier->supplier_alamat}}</textarea>
                    </div>
                    <div class="form-group">
                      <label class="col-form-label" for="suppliertelp"> No Telepon Supplier <a style="color:red">*</a></label>
                      <input type="text" class="form-control {{ $errors->has('suppliertelp') ? 'is-invalid' : '' }}" name="suppliertelp" id="suppliertelp" placeholder="Silahkan isi ..." maxlength="16" value="{{$supplier->supplier_telp}}">
                    </div>
                  </div>
                </div>
              </div>
